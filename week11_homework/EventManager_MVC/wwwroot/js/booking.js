...
function validateEventDate() {
    ...
    eventDate.setCustomValidity(
        eventDate.value && eventDate.value < eventDate.min
            ? "Etkinlik tarihi geçmiş bir gün olamaz."
            : ""
    );
}
...
eventDate.addEventListener("focus", validateEventDate);
...
function calculatePrice() {
    ...
    const validGuests = guestCount.value !== "" && Number.isInteger(guests)
        && guests >= Number(guestCount.min) && guests <= Number(guestCount.max);

    guestCount.setCustomValidity(validGuests ? "" : "Konuk sayısı 1 ile 500 arasında bir tam sayı olmalıdır.");
    if (!validGuests) {
